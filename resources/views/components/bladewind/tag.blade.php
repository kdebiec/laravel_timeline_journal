...
<label style="zoom:95%; background: {{$color}}" id="@if($add_id_prefix=='true')bw-@endif{{$id}}" class="text-xs uppercase px-[10px] py-[5px] tracking-widest whitespace-nowrap inline-block bg-{{$color}}-{{$color_weight[$shade]}} text-{{$color}}-{{$text_color_weight[$shade]}} {{$class}}">
{{ $label }}
@if($can_close == 'true')
<a href="javascript:void(0)" onclick="@if($onclick=='')this.parentElement.remove()@else{!!$onclick!!}@endif">
<svg style="background: {{$color}}" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 p-1 opacity-70 hover:opacity-100 !-mr-2 inline text-{{$color}}-{{$text_color_weight[$shade]}}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
